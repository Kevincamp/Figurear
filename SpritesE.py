...
def seisVertices2(posx,posy,color):
    global screen
    global puntos_malla
    global lista_linea
    global lista_puntos
    ...
    global puntos_lineas
    posicion_actual=(posx,posy)

    Puntos = [(-15,-26),(-30,0),(-15,26),(15,26),(30,0),(15,-26)]
    NuevoPuntos = []
    for valor in Puntos:
    ...
    while len(NuevoPuntos) != 0:
        ...
        if punto not in puntos_malla and PuntoEnPoligono(punto):
            ...
            for valor in Puntos:
                if PuntoEnPoligono((punto[0]+valor[0],punto[1]+valor[1])) :
                    v = (punto[0]+valor[0],punto[1]+valor[1])
                    if v not in puntos_malla:
                        ...
                        if not estaLinea((punto[0],punto[1]),(punto[0]+valor[0],punto[1]+valor[1])):
                            l=[(punto[0],punto[1]),(punto[0]+valor[0],punto[1]+valor[1])]
                            lista_linea.append(l)
                        #puntos_malla.append(v)
                        
                        
                else:
                    #pygame.draw.aaline(screen, color, (punto[0],punto[1]), pos, 1)
                    l=[(punto[0],punto[1]),pos]
                    lista_linea.append(l)
...
def estaLinea(p1 , p2):
    ...
    for punto in lista_linea:
        v1=punto[0]
        v2=punto[1]
        if (v1[0]==p1[0] and v1[1]==p1[1] and v2[0]==p2[0] and v2[1]==p2[1] ) or (v1[0]==p2[0] and v1[1]==p2[1] and v2[0]==p1[0] and v2[1]==p1[1] ):
            return True
    return False
        ...
        if punto not in puntos_malla and PuntoEnPoligono(punto):
            ...
            while indic < len(Puntos):
              ...
              if indic+1< len(Puntos):
                ...
                if PuntoEnPoligono((punto[0]+valor[0],punto[1]+valor[1])):
                    NuevoPuntos.append((punto[0]+valor[0],punto[1]+valor[1]))
                    if PuntoEnPoligono((punto[0]+valor2[0],punto[1]+valor2[1])):
                        #pygame.draw.aaline(screen, color, (punto[0],punto[1]), (punto[0]+valor[0],punto[1]+valor[1]), 1)
...
def encontrar_centroide(color):
    ...
    seisVertices2(posx,posy,color)
    print("encontro los primeros vertices")
    for punto in lista_linea:
        v1=punto[0]
        v2=punto[1]
        pygame.draw.aaline(screen, color, v1, v2, 1)
    pygame.display.flip()
    ...
    while ESTADO == "D":
        ...
        if e.type == pygame.MOUSEBUTTONUP :
            ...
            if PuntoEnPoligono(pos):
                ...
                bloque = Bloque([255,0,0], 10,10)
                bloque.rect.x = tachuela[0]-5
                bloque.rect.y = tachuela[1]-5
                screen.blit(bloque.image,( bloque.rect.x,bloque.rect.y))
                tachuelas.append(bloque)
                objetos.append(bloque)
                pygame.display.flip()
                ESTADO = "T"
...
def moverTachuelas():
    ...
    global ESTADO
    global screen
    global salirA,moverP,moverA,moverN,tachuelaP,tachuelaN,tachuelaA,dibujarP,dibujarA,dibujarN,nuevoA,nuevoN
    print("entra en funcion tachuela")
    draw_on = False
    if  len(tachuelas)<2: # tachuela radio 8
        print("entro a analizar tachuela")
        while ESTADO == "T":
            e = pygame.event.wait()
            #print(mouse , click[0])
            if e.type == pygame.MOUSEBUTTONDOWN :
                ...
                if pos[0]<= (x+8) and pos[0]>=(x-8) and pos[1]<= (y+8) and pos[1]>= (y-8):
